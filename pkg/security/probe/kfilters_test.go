...
func addRuleExpr(t *testing.T, re *rules.RuleEngine, exprs ...string) {
...
func TestIsParentDiscarder(t *testing.T) {
	...
	re := rules.NewRuleEngine(&Model{}, func() eval.Event { return &Event{} }, rules.NewOptsWithParams(model.SECLConstants, nil, enabled, nil, model.SECLLegacyAttributes, &log.PatternLogger{}))
	addRuleExpr(t, re, `unlink.file.path =~ "/var/log/*" && unlink.file.path != "/var/log/datadog/system-probe.log"`)

	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/var/log/datadog/system-probe.log"); is {
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/var/lib/datadog/system-probe.sock"); !is {
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/var/log/datadog/datadog-agent.log"); is {
		t.Error("shouldn't be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/var/lib/.runc/1234"); is {
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/etc/conf.d/nginx.conf"); is {
		t.Error("shouldn't be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/etc/sys.d/nginx.conf"); is {
		t.Error("shouldn't be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileRenameEventType, "rename.file.path", "/etc/conf.d/nginx.conf"); is {
		t.Error("shouldn't be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileRenameEventType, "rename.file.path", "/etc/nginx/nginx.conf"); !is {
		t.Error("should be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/etc/conf.d/abc"); is {
		t.Error("shouldn't be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileUnlinkEventType, "unlink.file.path", "/etc/cron.d/log"); is {
		t.Error("shouldn't be a parent discarder")
	}
	...
	if is, _ := isParentPathDiscarder(re, regexCache, model.FileOpenEventType, "open.file.path", "/tmp/runc"); is {
		t.Error("shouldn't be a parent discarder")
	}
}
...
func TestApproverAncestors1(t *testing.T) {
	enabled := map[eval.EventType]bool{"*": true}
	m := &model.Model{}
	re := rules.NewRuleEngine(m, m.NewEvent, rules.NewOptsWithParams(model.SECLConstants, nil, enabled, nil, model.SECLLegacyAttributes, &log.PatternLogger{}))
