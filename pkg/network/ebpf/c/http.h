...
static __always_inline void http_begin_response(http_transaction_t *http, const char *buffer) {
    http->response_status_code += (buffer[HTTP_STATUS_OFFSET+0]-'0') * 100;
    http->response_status_code += (buffer[HTTP_STATUS_OFFSET+1]-'0') * 10;
    http->response_status_code += (buffer[HTTP_STATUS_OFFSET+2]-'0') * 1;
}
...
static __always_inline http_transaction_t *http_fetch_state(http_transaction_t *http, skb_info_t *skb_info, http_packet_t packet_type) {
    ...
    http = bpf_map_lookup_elem(&http_in_flight, &http->tup);
...
static __always_inline int http_process(http_transaction_t *http_stack, skb_info_t *skb_info) {
    ...
    bool conn_closed = skb_info->tcp_flags & TCPHDR_FIN && http->owned_by_src_port == http_stack->owned_by_src_port;
    if (conn_closed) {
        to_flush = http;
    }
